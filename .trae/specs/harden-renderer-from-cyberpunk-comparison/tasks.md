...
- [x] Task 2: 启用被 App.tsx 硬编码禁用的组件
  - [x] SubTask 2.1: 在 `src/App.tsx` 中删除 5 处 `{false && ...}` 包装，恢复 HudPanel、UtilityMode、FpsCounter、ThemeSelector、ThemeEditor 渲染
  - [x] SubTask 2.2: 验证启用后 UI 不崩溃、tsc 通过、59/59 vitest 测试通过
...
- [x] Task 5: 重写 useFpsMonitor 为质量档策略
  ...
  - [x] SubTask 5.3: 更新 `src/App.tsx` 调用方适配新返回值结构（buildingCount + bloomEnabled）
...
- [x] Task 6: BloomEffect 接受 quality 控制
  ...
  - [x] SubTask 6.2: 在 `src/App.tsx` 中将 useFpsMonitor 的 bloomEnabled 传入 BloomEffect
...
- [x] Task 7: useSystemData 5s 超时切换
  ...
  - [x] SubTask 7.2: 在 `src/App.tsx` 中处理 `backend-unresponsive` 状态：渲染空城市 + fixed 定位 banner 提示
...
- [x] Task 9: 整体验证
  - [x] SubTask 9.1: 运行 `npx vitest run` — 13 文件 / 59 测试全部通过
  - [x] SubTask 9.2: 运行 `cargo check` — Finished dev profile，0 错误
  - [x] SubTask 9.3: 运行 `npx tsc --noEmit` — 0 错误（手动启动应用由用户完成）
